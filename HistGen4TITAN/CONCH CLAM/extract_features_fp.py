...
def compute_w_loader(output_path, loader, model, verbose = 0):
	...
	for count, data in enumerate(tqdm(loader)):
		with torch.inference_mode():	
			...
			#MY CHANGES
			# Always add TITAN attributes for REG2025
			if mode == 'w':
				attr_dict = {
...
if __name__ == '__main__':
	...
	print(f"Found {len(exist_idxs)} slides with valid h5 patch files out of {total} total")

	#Process only valid slides
	for index, bag_candidate_idx in enumerate(exist_idxs):
		...
		print(f'\nprogress: {index+1}/{len(exist_idxs)} (original idx: {bag_candidate_idx})')
		print(slide_id)

		# Double-check h5 file still exists
		if not os.path.exists(h5_file_path):
